...
Pair responses expose `holdout_provenance` separately from compatibility. New fit artifacts record that the current engine transforms/scales full input before splitting: this is blocked for training-only preprocessing qualification. Missing or unsupported records are unavailable; no record is silently certified. `decision_grade_ready` remains false pending a coordinated training-only preprocessing implementation and holdout access/reuse evidence.
